...
import { ownedCharmsPage } from '../mock/ownedCharms';

// 백엔드 연동 전까지 mock 사용. 연동 시 axios 호출만 살리면 됩니다.

/**
 * GET /api/account/owned-charms
 *
 * 응답:
 * {
...
export async function getOwnedCharms() {
  // const { data } = await api.get('/account/owned-charms');
  // return normalizeOwnedCharms(data);

  return Promise.resolve(normalizeOwnedCharms(ownedCharmsPage));
}
